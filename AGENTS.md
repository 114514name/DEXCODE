...
| **可视化 IDE(产品 B)** | `dexstudio/`(`host/` 11 个 C 文件 ~6300 行 + `web/` 11 个文件) | **DexStudio**:C 宿主 + 内嵌 **WebView2**(单窗口,HTML/CSS/JS 前端)。**模型层在 C**(`libdexstudio.dll`:项目/场景/撤销/视图/瓦片/节点图/命令通道),场景数据直接复用引擎的 `eng_scene_json` 与 `eng_comp_*`/`eng_field_*` 自省 → 引擎加组件不用改 IDE。**B3 场景 + B4 节点图 + B5 编译/运行 + B6 资源/自动保存 + B7 打包 + B8 中文编码 + B9 资源根 + B10 体验大修 + B12 交互修复** 已完成。前端只保留`场景`与`节点`两档，玩法统一由蓝图连线生成，不再提供积木/代码编辑页；字段元数据驱动的控件(枚举下拉/取色器/资源与实体下拉/只读标注)、父子层级树、场景管理、一键编译**先存盘**、生成前校验、资源改名同步引用、关窗未保存确认、单实例。测试:ctypes 调模型 + 无窗口 CLI + 离屏 WebView2 自测 + `node --check` + 页面自测 + 发布形态 + 中文路径 + 真实项目像素断言。细节/未做项见 `docs/DEXSTUDIO_UX_ISSUES.md`,见 `docs/DEXGAME_DESIGN.md` §9 |
...
| 提交 | 内容 |
|------|------|
| (本次 B12.5) | **「框线要适配图片大小、能拉能转」+ 拖动掉帧/图片比框线慢 —— 一个架构问题的两面**。① **框线改用"用户看到的东西"决定**:`scene.outline` 原来是**碰撞盒优先**(模板给玩家挂的 collider 是 32×32)→ 套上 300×400 的图,框线还是 32×32(用户原话"框线应该适配图片大小,而不是原来的示例");改成**精灵 > 碰撞体 > 瓦片地图**,碰撞盒单独给一份(`outline.collider`,前端画青色虚线)。② **换图按新图适配**:`comp.set sprite.tex_path` 里清掉"恰好 32×32"的旧模板裁切,并在用户**没手动缩放过**时按新图大小给合适缩放(长边 > 256 → 128),做了什么写进 `note` 弹出来。③ **引擎真的支持旋转了**:新增 `dg_draw_quad_corners`(四角自己给)+ sprite 绕**轴心**旋转(`transform.rot` 单位**度**、屏幕顺时针为正);`sprite.visible` 成正式字段(默认 1);宿主 `scene.outline` 用**同一公式**算出四角/`rot`/`pivot` 一起给前端 —— 前端只画 C 给的四角,不写第二遍。④ **选择框变成真手柄**:8 个缩放手柄 + 1 个旋转手柄(Shift 等比/吸附 15°;拉伸时**对边固定**;拖完 `comp.set_many` **一条撤销**),`transform.rot/sx/sy` 从只读变可编辑。⑤ **拖动不卡了**:`requestDraw()` rAF 合帧 + "预览图+网格"离屏缓存 + `view.set` 搭渲染那拍发(以前每个 `mousemove` 都重画整张图、都发一条命令)。⑥ **图片与框线同步**:按下时让引擎把被拖的精灵**藏起来**(`scene.render {hide:[…]}`,渲染完立刻还原、不动场景数据),画布上自己画一份**跟手**的贴图(仿射矩阵映射到四角,支持旋转/翻转/染色 + 选中时预热解码)—— **拖动过程里一条命令、一次引擎渲染都不发**,松手才写回。**测**:`tests/test_dexstudio.py` 490 → **508 项**(新增 `test_sprite_transform`:转 90° 时框线外接矩形 = 画面像素包围盒、`visible=0` 一个像素都不画、`hide` 渲染完 `visible` 必须还原;页面自测 +6 条:手柄存在/预览四角与 C 一致/拖动 0 命令/松手写回预览那份/一次撤销/Shift 吸附),`--wv-selftest` 页面自测 **97 项 0 失败**。**零字节码格式改动**(引擎只加了一个内部绘制函数与一个 sprite 字段) |
...
| (本次 B12) | **DexStudio 交互与蓝图收敛修复**。资源扩展名大小写统一识别；资源缩略图读取失败时在资源项中显示明确错误并记录原因。视口缩放到网格过密时只隐藏网格、保留原点轴；移动预览使用与最终 transform 相同的网格吸附和包围框偏移，避免预览位置与最终位置不符。节点拖动改为 `requestAnimationFrame` 合帧绘制，减少卡顿。编译统一从 `scripts/logic.json` 生成，不再被旧积木模式覆盖；前端只保留场景/节点蓝图入口，隐藏积木与代码编辑功能。 |
