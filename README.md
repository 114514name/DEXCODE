...
# 9. (可选)DexStudio 可视化 IDE(产品 B:B1 工具链 + B2 宿主 + B3 场景 + B4 逻辑 + B5 编译/运行已完成)
...
python tests/test_dexstudio.py                   # 251 项(含 WebView2 整条链 + 页面自测)
...
**B5 接上了编译与运行**:一键「编译」= 存盘 + 重生成逻辑图 + `dexc.exe`,
诊断被解析成行列号,在输出面板里点一下就能跳到「代码」页签并在那一行闪一下;
「运行」用无控制台版 VM 开**独立游戏窗口**(可「停止」)。代码页签的高亮是自写的
词法着色(零第三方 JS)。
...
python tests/test_dexstudio.py   # 251 项:DexStudio 模型层 + 逻辑图 + 编译/运行 + CLI + WebView2 + 页面自测
...
- **运行期不释放字符串**(结构体已可安全释放,见下)：字符串拼接(`CONCAT`)与原生函数
  返回的字符串在进程退出前不会释放。因此 `while ... { s = s + "x"; }` 这类循环会持续
  增长内存(且是 O(n²):每轮泄漏的是上一轮的完整累加串)。
  **缓解**:VM 有内存预算机制 —— 设置环境变量 `DEXCODE_MAX_MEM_MB`(单位 MB,`0`=不限,
  默认不限)后,用量超过上限会以明确错误退出,并指出"循环内字符串累加"这一最常见原因,
  而不是静默涨到被系统 OOM 杀掉。分配已收口到 `vm_alloc`/`vm_calloc`,是引入回收策略的
  单一插入点。`tests/test_robust.py::test_string_flow_aliasing` 固化了当前行为。
  字符串引用计数是 `docs/MEMORY_DESIGN.md` 的 P4,尚未实施。
- **结构体是值语义**(P3 起)：`let b = a;` 会把结构体深拷贝一份,改 `b.x` 不影响 `a.x`;
  传参同理(形参是副本)。这与源码表面写法一致(`Box{...}`、`b.f`,没有取地址、没有 `null`)。
  配套地,**递归类型被拒绝**(`type N { self: N; }` 报错)——因为值类型不能包含自身
