...
- **字符串累加是 O(n²) 时间**(内存已回收,不是泄漏)：字符串拼接(`CONCAT`)每次都必须
  复制整个累加串,因此 `while ... { s = s + "x"; }` 20,000 次约需 0.5 s。内存方面已由
  引用计数解决(P4,实测 200,000 次仅 5.1 MB,此前约 13.6 GB),**不再增长**;
  但时间上仍应避免反复拼接同一变量 —— 需要累积大量文本时建议写入文件
  (`dex_write_file` / `dex_append_file`)。
  **预算机制**:仍可设置环境变量 `DEXCODE_MAX_MEM_MB`(单位 MB,`0`=不限,默认不限),
  超限时给出可诊断的错误并从"循环内字符串累加"等常见原因开始提示。
